...
class RunWorker {
  ...
  startClusterServer(script, data) {
    ...
    var child = childProcess.fork(__dirname + '/' + script, [serverCount, JSON.stringify(data)], e => {
      if (e) console.log(e);
    });
  ...
  startClient(data) {
    var child = childProcess.fork(__dirname + '/startClient', [0, JSON.stringify(data)], e => {
      if (e) console.log(e);
    });
